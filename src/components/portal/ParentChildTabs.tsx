...
export function ParentChildTabs({ data, childName, childClass, childSection, childRollNumber }: any) {
    ...
    const tabs = [
        { id: 'ebooks', label: 'E-Books', icon: FileText, color: 'text-emerald-500', bg: 'bg-emerald-500', border: 'border-border' },
        ...
        { id: 'attendance', label: 'Attendance', icon: CalendarIcon, color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-900/200', border: 'border-amber-200' },
        ...
        switch (activeTab) {
                ...
                return (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        {data.materials.length === 0 ? (
                            <div className="p-8 text-center text-muted-foreground text-sm bg-card rounded-xl border border-dashed border-border">No E-Books available currently.</div>
                        ) : (
                            <div className="grid gap-3 sm:grid-cols-2">
                                {data.materials.map((m: any) => (
                                    <div key={m.id} className="p-4 flex items-center justify-between bg-card border border-border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                        <div>
                ...
                return (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        {data.homeworks.length === 0 ? (
                            <div className="p-8 text-center text-muted-foreground text-sm bg-card rounded-xl border border-dashed border-blue-200">No assignments mapped.</div>
                        ) : (
                            <div className="grid gap-4">
                                {data.homeworks.map((h: any) => {
                                    const isOverdue = new Date(h.dueDate) < new Date();
                                    return (
                                        <div key={h.id} className="p-4 bg-card border border-border rounded-xl shadow-sm">
            ...
            case 'homework':
                return (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        {data.homeworks.length === 0 ? (
                            <div className="p-8 text-center text-muted-foreground text-sm bg-card rounded-xl border border-dashed border-indigo-200">No active homework pending.</div>
                        ) : (
                            <div className="grid gap-3 sm:grid-cols-2">
                                {data.homeworks.map((h: any) => {
                                    const isOverdue = new Date(h.dueDate) < new Date();
                                    return (
                                        <div key={h.id} className="p-4 bg-card border border-border rounded-xl shadow-sm relative overflow-hidden">
                                            {isOverdue && <div className="absolute top-0 right-0 w-2 h-full bg-red-400" />}
                                            <div className="font-bold text-foreground pr-4">{h.title}</div>
                ...
                return (
                    <div className="space-y-4 animate-in fade-in duration-300">
                        {(!data.examResults || data.examResults.length === 0) ? (
                            <div className="p-8 text-center text-muted-foreground text-sm bg-card rounded-xl border border-dashed border-purple-200">No performance records available.</div>
                        ) : (
                            <div className="grid gap-3 sm:grid-cols-2">
                                {data.examResults.map((r: any) => (
                                    <div key={r.id} className="p-4 bg-card border border-border rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <div className="font-bold text-sm text-foreground">{r.exam.title}</div>
                                        ...
                                        {r.remarks && <p className="text-xs text-muted-foreground mt-2 italic">"{r.remarks}"</p>}
                                    </div>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="space-y-6 animate-in fade-in duration-300">
                        <div className="bg-card p-6 rounded-xl border border-border flex items-center justify-between shadow-sm">
                            <div className="flex flex-col">
                                <span className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Attendance Status</span>
                                <span className="text-foreground text-sm mt-1">Total recorded presence for this semester</span>
                            </div>
                        ...
                        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                            <div className="p-4 border-b border-amber-50 bg-amber-50 dark:bg-amber-900/20/30">
                                ...
                                <table className="w-full text-sm text-left">
                                    <tbody className="divide-y divide-amber-50">
                                        {DAYS.map((day, idx) => {
                                            ...
                                            return (
                                                <tr key={idx} className="hover:bg-amber-50 dark:bg-amber-900/20/20">
                                                    <td className="px-5 py-3 font-bold text-amber-800 bg-amber-50 dark:bg-amber-900/20/50 w-[120px]">{day}</td>
                                                    <td className="px-5 py-3">
                                                        <div className="flex flex-wrap gap-2">
                                                            {daySlots.map((slot: any) => (
                                                                <div key={slot.id} className="bg-amber-50 dark:bg-amber-900/20 border border-border rounded p-2 min-w-[120px]">
                            ...
                            <div className="p-4 border-t border-border bg-card">
                                ...
                                <div className="space-y-2">
                                    {data.attendancesList?.map((att: any) => (
                                        <div key={att.id} className="flex justify-between items-center p-2 rounded bg-amber-50 dark:bg-amber-900/20/50">
            ...
            case 'fee':
                return (
                    <div className="space-y-6 animate-in fade-in duration-300">
                                ...
                                <div className="space-y-3">
                                    {data.feeChallans.filter((f: any) => f.status === 'Pending' || f.status === 'Overdue').map((f: any) => (
                                        <div key={f.id} className="flex justify-between items-center p-3 rounded-lg border border-rose-50 bg-rose-50/30">
                                            <div>
                                                <div className="font-bold text-sm text-foreground">{f.month} {f.year} - {f.challanNumber}</div>
                                                <div className={`text-xs mt-1 font-semibold ${f.status === 'Overdue' ? 'text-red-500' : 'text-muted-foreground'}`}>
                        ...
                        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                            <h3 className="font-bold text-foreground mb-4 text-xs uppercase tracking-wider">Accounts for Payment</h3>
                            {!data.bankAccounts || data.bankAccounts.length === 0 ? (
                                <p className="text-sm text-muted-foreground">No bank accounts configured. Please contact the school.</p>
                            ) : (
                                <div className="grid gap-3 sm:grid-cols-2">
                                    {data.bankAccounts.map((acc: any) => (
                                        <div key={acc.id} className="p-4 rounded-lg bg-slate-50 border border-slate-100">
                                            <div className="text-[10px] uppercase text-muted-foreground font-bold mb-1">{acc.bankName}</div>
                                            <div className="font-bold text-sm text-foreground">{acc.accountTitle}</div>
                                            <div className="font-mono text-xs text-muted-foreground mt-1">{acc.accountNumber}</div>
                                        </div>
                                    ))}
                                </div>
                            )}
    ...
    return (
        <div className="space-y-6 bg-muted/50 p-6 rounded-2xl border border-border shadow-sm">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-border pb-6">
                <div>
                    <h2 className="text-2xl font-black text-foreground">{childName}</h2>
                    <div className="text-muted-foreground font-semibold flex gap-2 items-center mt-2">
                        ...
                        <span className="bg-muted text-muted-foreground px-2 py-0.5 rounded text-xs font-mono">Roll: {childRollNumber}</span>
                    </div>
                </div>
            </div>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all duration-200
                                ${isActive
                                    ...
                                    : `bg-card text-muted-foreground hover:bg-muted border border-border hover:shadow-sm`
                                }
                            `}
                        >
